...
def iterative_classifier_wrapper(sampleName):
    ...
    while True:  #while there is more than a 20% difference b/w old & new classifications, keep iterating
        ...
        for i in range(1, x-1):
            for j in range(1, y-1):
                classifications[i, j] = new_classifications[i, j]
        if pcnt_diff <= 0.1:
            break
    ...
    if z <= 6:
        ...
        for i in range(x):
            for j in range(y):
                scan[i, j, 6] = classifications[i, j]
        scan = scan.reshape((x2, z))
        np.savetxt('../Data/AFM/AggregatedData/%s.txt'%(sampleName), scan) 
...
def iterative_classifier_comparison(classifications, coords):
    ...
    for i in range(x-2):    #put the newly randomized coordinate pairs into 2D array
        for j in range(y-2):
    ...
    for i in range(x-2):  #store neighbor locations of the randomly selected pixel. Initialize pixel class.
        for j in range(y-2):
            neighbors = neighbor_locater(coord_array[i, j, 0], coord_array[i, j, 1])
            pixel_class = classifications[i+1, j+1]
        
            for k in range(8): #retrieve and store the classifications of the neighboring pixels
            ...
            new_classifications[i+1, j+1] = iterative_classifier_assignment(pixel_class, neighbor_classes)
        
    num_diff_pixels = 0
    
    for i in range(x-2):
        for j in range(y-2):
            if new_classifications[i+1, j+1] == classifications[i+1, j+1]:
                pass
            else:
                    num_diff_pixels += 1
